...
*/
public class RecipientList<T> {
    public typealias MessageHandler = T->()
    
    /// The current list of recipients.
    private var entries = [RecipientListEntry<T>]()
    ...
    */
    public func add(function: MessageHandler) -> RecipientListEntry<T> {
    ...
    - returns: An opaque object that can be used to stop any further messages.
    */
    public func add<R: Recipient>(object: R, _ function: R->MessageHandler) -> RecipientListEntry<T> {
        let entry = RecipientListEntry(object: object, function: { function($0 as! R) })
        sync {
            self.entries += [entry]
        }
        return entry
    ...
    */
    public func remove(entry: RecipientListEntry<T>) {
        sync {
    ...
    */
    internal func copyEntries() -> [RecipientListEntry<T>] {
        var entries = [RecipientListEntry<T>]()
...
public class RecipientListEntry<T> {
